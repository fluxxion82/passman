...
class LocalUserRepository(
    ...
    /**
     * A new account is the device keyring, the identity store, an empty vault and a credential —
     * nothing else. No PGP rings and no starter keystore are created here or anywhere else on the
     * signup path: both used fixed filenames, so two devices signing into one profile minted
     * different files under the same names and the first sync overwrote one of them, taking a
     * secret ring with it. The user creates keys and keystores on the Create screens, and a second
     * device inherits them by syncing.
     */
    override suspend fun signup(username: String, password: String): Outcome<AppUser> = withContext(coroutinesContextFacade.io) {
        ...
        passmanSessionScope(userPreferences.getSessionId()) { scope ->
            bootstrapAccount(scope, username, password)
    ...
    /**
     * Create a brand-new account: keyring, identity store, warmed session keys, vault, credential.
     * Every step after the keyring exists rolls the whole account back on failure — including a
     * throw — so a retry starts from nothing rather than from a half-built account.
     *
     ...
     * `try`.
     *
     * The steps used to be extensible, through an `afterIdentityStore` / `beforeCommit` lambda pair,
     * so that signup could start PGP ring generation next to the vault init and await it just before
     * the commit. Nothing is provisioned on signup any more, so the hooks had no suppliers left and
     * the sequence is written out straight.
     */
    private suspend fun bootstrapAccount(
        ...
        try {
            ...
            warmIdentityKeys(scope, username, storePassword.value)
